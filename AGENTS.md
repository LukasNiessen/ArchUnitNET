...
**ArchUnitNET** is the C# member of the ArchUnitEverything family: one architecture-testing library
per language, all recognisably the same product. Users write architecture rules as ordinary unit
tests.

```csharp
[Fact]
public void ApiDoesNotTouchTheDatabase()
{
...
production, plus ArchUnitGo, ArchUnitRuby and ArchUnitRust, which are being built alongside this one.
...
**Follow it by default. Where C# makes a convention awkward, diverge — knowingly, and leave a
line in the commit message or a comment saying why.** Fighting the host language to match a sibling
is the one failure mode worse than divergence. Idiomatic C# wins whenever the two genuinely
...
| Stage | Does what | C#-specific? |
|---|---|---|
| **Source** | The user's project, found from a project root or a build file. | Only in how the root is found. |
| **Extract** | Walk and parse the source, resolve every import to a target, emit `Edge` values. | **Yes, entirely.** This is the only stage that knows C#. |
...
The consequence worth internalising: **almost all the C#-specific work is in Extract.** If you
...
```
src/ArchUnitNet/Common/         the kernel — everything shared
src/ArchUnitNet/  Extraction/   the dependency-graph extractor  <- the only C#-specific code
src/ArchUnitNet/  Projection/   reshaping the graph, plus Cycles/
src/ArchUnitNet/  Assertion/    Violation, EmptyTestViolation
src/ArchUnitNet/  FluentApi/    Checkable, CheckOptions
src/ArchUnitNet/  Error/        TechnicalError, UserError
src/ArchUnitNet/  Util/         logging, pattern matching, path helpers
src/ArchUnitNet/Files/          file-level dependency and naming rules
src/ArchUnitNet/Layers/         named-layer policy
src/ArchUnitNet/Slices/         component and diagram rules
src/ArchUnitNet/Metrics/        numeric code-quality rules
src/ArchUnitNet/Graph/          dependency-graph reports
src/ArchUnitNet/Testing/        violation formatting and test-framework glue
src/ArchUnitNet/ArchUnit.cs     the public surface — re-exports, nothing else
```
...
1. `common` depends on nothing but the standard library and the C# analysis toolchain.
...
Six types carry everything. Keep the names; adapt the shapes to whatever C# wants.
...
- **Zero matches is a violation, not a pass.** A selector matching no files is almost always a typo,
  ...
  terminal needs it.
- `ImportKind` is **expected to differ per language**. Here it is `Using`, `StaticUsing`, `GlobalUsing`, `AliasUsing`, `ExternAlias`. This is the one
...
architect who does not write C# would understand, the name is wrong.** Not the implementation —
the name.
...
Word choice is fixed. Casing is yours. Everything public is `PascalCase`. `depend on files` becomes `DependOnFiles`.
...
| Thing | Convention |
|---|---|
| Directories | Lowercase, singular, unabbreviated, no separators. `fluentapi` is one word. `PascalCase` — `Common`, `Files`, `FluentApi`, `Assertion`. The canonical names are lowercase; C# folders are not, and that is fine |
| Files | One concept per file, named after the concept: `ExtractGraph.cs`, one public type per file, named after the type |
...
- **Solution** `ArchUnitNET.sln`, library in `src/ArchUnitNet/`, tests in
  `tests/ArchUnitNet.Tests/`. Namespaces mirror folders: `ArchUnitNet.Common.Extraction`,
  `ArchUnitNet.Files.FluentApi`.
- **We analyse source, not assemblies.** Extraction uses Roslyn
  (`Microsoft.CodeAnalysis.CSharp`) over the project's syntax trees. There is an older, unrelated
  `ArchUnitNET` in the ecosystem that reflects over compiled assemblies — we deliberately do not do
  that, because source analysis is what the whole family does and what keeps the rules portable.
- **Locate the project** via `.sln` or `.csproj`, walking up from the working directory.
- **Options bags** are records with init-only properties and defaults:
  `Check(CheckOptions? options = null)`.
- **Errors** are `TechnicalError` and `UserError`, both `: Exception`. A failing rule is an
  `IReadOnlyList<Violation>` — never a throw.
- **Testing.** A FluentAssertions extension gives `rule.Should().Pass()` for xUnit; a plain
  `ArchAssert.Passes(rule)` covers NUnit, MSTest and anything else with no configuration.
- **Async.** Roslyn is async-first, so terminals may return `Task`. If you go async, ship
  `CheckAsync` and keep every terminal symmetric — do not mix.
- **Node vocabulary.** C# users think in types and namespaces at least as much as in files. Files
  stay primary for parity; type-level selectors are the obvious extension.
- Nullable reference types on, warnings as errors, analyzers enabled.
...
- Issues in this repository cover a large chunk of the planned work. They are starting points, not
  specifications. If one conflicts with reality in C#, say so and we adjust it.
